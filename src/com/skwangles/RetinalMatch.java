...
//        equalizeHist(src2, src2);
        ...
        compareCleanedRetinas(src1, src2, src1path, src2path, blackWhite1, blackWhite2);

    }


    private static void compareCleanedRetinas(Mat src1, Mat src2, String src1path, String src2path, Mat mask1, Mat mask2){
        double matchThreshold = 0.99995;


        //Create image histograms for comparison
        List<Mat> hsvBaseList = Arrays.asList(src1);
        Imgproc.calcHist(hsvBaseList, new MatOfInt(0), mask1, src1, new MatOfInt(new int[]{10}), new MatOfFloat(0, 256), false);
        Core.normalize(src1, src1, 0, 1, Core.NORM_MINMAX);

        List<Mat> hsvBaseList2 = Arrays.asList(src2);
        Imgproc.calcHist(hsvBaseList2, new MatOfInt(0), mask2, src2, new MatOfInt(new int[]{10}), new MatOfFloat(0, 256), false);
